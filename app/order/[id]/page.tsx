      ...
      <section className="mx-auto max-w-4xl px-5 py-16 md:px-8">
        <div className="rounded-lg border border-black/10 bg-white p-6 shadow-[0_24px_70px_rgba(16,35,29,0.08)] md:p-8">
          ...
          <div className="mt-4 flex flex-col justify-between gap-4 md:flex-row md:items-start">
            <div>
              <h1 className="text-4xl font-black tracking-tight text-[#10231D] md:text-5xl">
                {order.id}
              </h1>
              <p className="mt-3 text-sm font-semibold text-[#5F6B66]">
                Submitted {formatDate(order.createdAt)}
              </p>
            </div>
            <span className="w-fit rounded-full bg-[#E9F2EE] px-4 py-2 text-sm font-black text-[#0B4D3B]">
              {order.status}
            </span>
          </div>

          <p className="mt-6 rounded-lg bg-[#F5F7F4] p-4 text-sm font-bold leading-7 text-[#10231D]">
            {statusCopy[order.status]}
          </p>

          {!canViewPrivateDetails ? (
            <div className="mt-6 rounded-lg border border-[#F4DEAE] bg-[#FFF9EA] p-5">
              <h2 className="text-lg font-black text-[#10231D]">Private details protected</h2>
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                <Link
                  href={loginPath}
                  className="inline-flex h-11 items-center rounded-full bg-[#0B4D3B] px-5 text-sm font-bold text-white transition hover:bg-[#D4AF37] hover:text-[#10231D]"
                >
                  Sign in to view details
                </Link>
                <Link
                  href={registerPath}
                  className="inline-flex h-11 items-center rounded-full border border-[#0B4D3B] px-5 text-sm font-bold text-[#0B4D3B] transition hover:bg-white"
                >
                  Create account
                </Link>
              </div>
            ...
            <>
              <div className="mt-8 grid gap-5 md:grid-cols-2">
                <div className="rounded-lg border border-black/10 p-5">
                  <h2 className="text-lg font-black text-[#10231D]">Customer</h2>
                  <div className="mt-4 grid gap-2 text-sm leading-7 text-[#5F6B66]">
                ...
                <div className="rounded-lg border border-black/10 p-5">
                  <h2 className="text-lg font-black text-[#10231D]">Delivery</h2>
                  <div className="mt-4 grid gap-2 text-sm leading-7 text-[#5F6B66]">
                    <span>{order.delivery}</span>
                    <span>{order.payment}</span>
                    <span className="font-black text-[#0B4D3B]">{order.total}</span>
                  </div>
                </div>
              </div>

              <div className="mt-5 rounded-lg border border-black/10 p-5">
                <h2 className="text-lg font-black text-[#10231D]">Payment</h2>
                <div className="mt-4 grid gap-2 text-sm leading-7 text-[#5F6B66] md:grid-cols-2">
              ...
              <div className="mt-5 rounded-lg border border-black/10 p-5">
                <h2 className="text-lg font-black text-[#10231D]">Items</h2>
                <p className="mt-4 whitespace-pre-line text-sm leading-7 text-[#5F6B66]">{order.order}</p>
              </div>
            </>
          ) : null}
