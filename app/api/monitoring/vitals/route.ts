 ...
 *
 * One request carries a page's vitals together ({ metrics: [...] }) and they
 * are written in one statement: five requests and five INSERTs per page view
 * were five wake-ups of a database that bills for each. A single vital in the
 * old shape is still accepted, from a tab opened before this deploy.
 */
...
/** A page has five vitals; anything much longer is not a page. */
const MAX_PER_REQUEST = 10;

type Vital = { path: string; method: string; metric: string; duration: number; statusCode: number };

function vitalFrom(raw: unknown): Vital | null {
  ...
  if (!Number.isFinite(value) || value < 0 || value > MAX_VALUE_MS) return null;
  ...
  if (!path.startsWith("/") || path.includes("://") || path.length > MAX_PATH) return null;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const raw: unknown[] = Array.isArray(body?.metrics) ? body.metrics : [body];
    const vitals = raw
      .slice(0, MAX_PER_REQUEST)
      .map(vitalFrom)
      .filter((vital): vital is Vital => vital !== null);

    // Nothing valid, nothing written — and the database is not woken for it.
    if (vitals.length > 0) {
      await logPerformanceMetrics(vitals);
    }
    ...
    // internet. logPerformanceMetrics swallows its own failures already.
    return new NextResponse(null, { status: 204 });
  }
}
