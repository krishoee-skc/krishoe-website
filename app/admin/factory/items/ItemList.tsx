...
// One shape for an item and for a production item, defined where they are read.
import type {
  FactoryItem,
  ProductionItemOption as ProductionItem,
...
/**
 * The item screen: what the factory makes, what each item is linked to in the
 * production ledger, and what a pair of it costs in material.
 *
 * The server hands over the list, retired items included — this is the only
 * screen that can bring one back. Every save reloads through the API, which
 * reads the very same query.
 */
export default function ItemList({
  initialItems,
  initialProductionItems,
}: {
  initialItems: FactoryItem[];
  initialProductionItems: ProductionItem[];
}) {
  const { text } = useLanguage();
  const [items, setItems] = useState<FactoryItem[]>(initialItems);
  const [productionItems, setProductionItems] = useState<ProductionItem[]>(initialProductionItems);
