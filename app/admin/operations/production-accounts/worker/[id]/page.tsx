  ...
  return (
    <section className="mx-auto max-w-6xl space-y-5 p-4 pb-28 sm:p-6">
      ...
      <div className="rounded-2xl border border-brand-green-line bg-brand-paper p-4 shadow-sm sm:p-5">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
          {[
            ...
            <div key={label} className="rounded-xl bg-brand-paper-deep p-3">
      ...
      <form action={createWorkerPaymentAction} className="rounded-2xl border border-emerald-200 bg-brand-paper p-4 shadow-sm sm:p-5">
        ...
        <label className="mt-4 flex min-h-12 items-center gap-3 rounded-xl bg-brand-paper-deep px-3 text-sm font-bold text-brand-green-ink">
      ...
      <div className="grid gap-5 xl:grid-cols-2">
        <div className="rounded-2xl border border-brand-green-line bg-brand-paper p-4 shadow-sm sm:p-5">
          ...
          <div className="mt-4 space-y-3">
            {account.work.map((row) => (
              <article key={row.id} className="rounded-xl border border-brand-green-line p-3 text-sm">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="font-black text-brand-green-ink">{row.itemName} · {row.stage}</p>
                    <p className="mt-1 text-brand-muted">{row.workDate} · {row.totalPairs} pairs · rate {money(row.ratePerPair)}</p>
                  ...
                  <p className={`font-black ${row.status === "Reversed" ? "text-brand-muted-soft line-through" : "text-brand-green"}`}>
                    {money(row.earnedWage)}
                  </p>
                </div>
                {row.status === "Reversed" ? (
                  <p className="mt-2 rounded-lg bg-brand-mist px-2 py-1 text-xs font-black text-brand-muted">
            ...
            {account.work.length === 0 ? <p className="text-sm text-brand-muted">No work entry yet.</p> : null}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-brand-green-line bg-brand-paper p-4 shadow-sm sm:p-5">
          ...
          <div className="mt-4 space-y-3">
              ...
              <article key={row.id} className="rounded-xl border border-brand-green-line p-3 text-sm">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-brand-muted">{row.paymentDate} · Cash · {row.receiptNumber}</p>
            ...
            {account.payments.length === 0 ? <p className="text-sm text-brand-muted">No cash or adjustment entry yet.</p> : null}
          </div>
        </div>
      </div>
    </section>
